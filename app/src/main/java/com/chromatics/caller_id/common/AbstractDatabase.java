...
public abstract class AbstractDatabase<T extends AbstractDatabaseDataSlice<V>, V> {
    ...
    protected Map<Integer, T> sliceCache = new HashMap<>(); // TODO: cache cleaning
    ...
    protected void loadSlice(T slice, String fileName, boolean useInternal) {
        try {
            loadSliceUnsafe(slice, fileName, useInternal);
        } catch (Exception e) { // TODO: handle inconsistent slice state (load failure)
            LOG.warn("loadSlice()", e);
        }
        LOG.trace("loadSlice() finished");
    }

    protected void loadSliceUnsafe(T slice, String fileName, boolean useInternal) throws IOException {
        LOG.debug("loadSliceUnsafe() started with fileName={}, useInternal={}", fileName, useInternal);
